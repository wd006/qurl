document.addEventListener('DOMContentLoaded', function() {
    ...
    if (!protocolToggle) {
        console.error("ERROR(advanced-settings.js): checkbox 'protocolToggle' not found");
        return;
    }
    if (!protocolDisplay) {
        console.error("ERROR(advanced-settings.js): span 'protocol-display' not found"); // TODO: rename to protocolDisplay
        return;
    }

    console.log("INFO(advanced-settings.js): elements found");

    function updateProtocol() {
        if (protocolToggle.checked) {
            protocolDisplay.textContent = 'https://';
            protocolDisplay.style.color = '#3b82f6';
            console.log("INFO(advanced-settings.js): selected HTTPS");
        } else {
            protocolDisplay.textContent = 'http://';
            protocolDisplay.style.color = '#bbb';
            console.log("INFO(advanced-settings.js): selected HTTP");
        }
    }
